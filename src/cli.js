...
function pluginManifest(name) { const file = path.join(root, 'plugins', name, 'harness-plugin.yaml'); if (!exists(file)) throw new Error(`Plugin manifest not found: ${name}`); return readText(file); }
...
try {
  ...
  else if (command === 'remove') { const config = parseConfig(); delete config.plugins[positional[0]]; saveConfig(config); console.log(`Removed plugin ${positional[0]}`); }
